...
(deftest get-edn-files-test
  (let [path (file/get-edn-files "test/riemann/discovery/edn/")]
    (is (= 2 (count path)))
    (is (some #{"test/riemann/discovery/edn/f1.edn"} path))
    (is (some #{"test/riemann/discovery/edn/f2.edn"} path))))

(deftest read-edn-file-test
  (is (= (file/read-edn-file "test/riemann/discovery/edn/f1.edn")
         [{:ttl 120
...
(deftest read-edn-files-test
  (let [result (file/read-edn-files ["test/riemann/discovery/edn/"])]
    (is (= (count result) 3))
    (is (some #{{:ttl 120
                 :services [{:hosts ["kafka1" "kafka2"]
                             :name "kafka"
...
(deftest file-discovery-test
  (with-mock [calls riemann.discovery.discovery/reinject-events]
    (let [d (file/file-discovery {:path ["test/riemann/discovery/edn/"]})]
      (is (= (count @calls) 0))
      (advance! 29)
      (is (= (count @calls) 0))
      (advance! 30)
